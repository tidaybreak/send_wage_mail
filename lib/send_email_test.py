...
def read_data(excel_file):
    ...
    for row in ws.rows:
        ...
        if not first_line:
            data.append(item)
        first_line = False
    return note, titles, data, ws.merged_cells, staff_rows
...
def main():
    ...
    note, titles, salary_data, merged_cells, staff_rows = read_data(current_dir + os.sep + '../money.xlsx')
    html_template = '<table border="1" width="2200" style="border-collapse:collapse">'
    html_template += '<thead>'
    html_template += '<tr><th colspan="21" style="font-size:20px;padding-top:20px;padding-bottom:20px;">%s' % note
    html_template += '</th></tr>'
    html_template += '<tr>'
        ...
        for item in salary_data[row_index:row_index + staff_row]:
            ...
            for i in item[1:]:
                ...
                if check["type"] == 'rowspan':
                    holder_str += '<td width="%s" style="padding-left:10px;padding-right:10px;" rowspan="%s">%s</td>' % (
                ...
                if check["type"] == 'normal':
                    holder_str += '<td width="%d" style="padding-left:10px;padding-right:10px;">%s</td>' % (width, val)
            holder_str += '</tr>'
        html_content = html_template.replace('<<placeholder>>', holder_str)
        if staff_email is not None:
            if staff_email != 'test@ofidc.com':
                time.sleep(5)
                send_result = send_mail(staff_email, "群发测试", "群发测试", user, pwd, server, port, enable_ssl)
                if not send_result:
                    has_failture = True
